...
static void powerDistributionLegacy(const control_t *control, motors_thrust_uncapped_t* motorThrustUncapped)
{
  ...
  {
    motor[0] = ZERO_THRUST + control->thrust / 2.0f - control->velocity.y / 10.0f;
    ...
    motor[3] = ZERO_THRUST + control->thrust / 2.0f + control->velocity.y / 10.0f;
  }
  ...
  motorThrustUncapped->motors.m2 = motor[1];
  motorThrustUncapped->motors.m3 = motor[2];
